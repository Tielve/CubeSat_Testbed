...
void readMPUAccel(float *gx, float *gy, float *gz)
{
	...
	*gx = ((float)x / mpu_accel_scale);
	*gy = ((float)y / mpu_accel_scale);
	*gz = ((float)z / mpu_accel_scale);
}
...
void readMPUGyro(float *gx, float *gy, float *gz)
{
	...
	*gx = ((float)x / mpu_gyro_scale) * (M_PI / 180.0f);
	*gy = ((float)y / mpu_gyro_scale) * (M_PI / 180.0f);
	*gz = ((float)z / mpu_gyro_scale) * (M_PI / 180.0f);
}
